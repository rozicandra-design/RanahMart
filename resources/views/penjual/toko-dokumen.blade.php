...
<style>
    ...
    .dok-wrap { background: #f6f5f2; min-height: 100vh; padding: 28px; }

    .status-banner { border-radius: 16px; padding: 16px 20px; margin-bottom: 20px; display: flex; align-items: flex-start; gap: 14px; }
    ...
    .card { background: #fff; border-radius: 20px; border: 1px solid #ebebeb; padding: 26px; box-shadow: 0 1px 4px rgba(0,0,0,.04); }
    .card + .card { margin-top: 20px; }

    .section-title { font-size: 11px; font-weight: 800; color: #bbb; text-transform: uppercase; letter-spacing: .7px; display: flex; align-items: center; gap: 8px; margin-bottom: 18px; }
    ...
    .field-label { display: flex; align-items: center; gap: 6px; font-size: 11px; font-weight: 800; color: #555; text-transform: uppercase; letter-spacing: .5px; margin-bottom: 7px; }
    .field-ok { font-size: 10.5px; font-weight: 700; color: #0d9488; background: #f0fdfa; border: 1px solid #99f6e4; padding: 1px 8px; border-radius: 999px; text-transform: none; letter-spacing: 0; }

    .input-field { width: 100%; border: 1.5px solid #e8e8e8; border-radius: 11px; padding: 11px 14px; font-size: 13.5px; color: #1a1a1a; font-family: 'Plus Jakarta Sans', sans-serif; background: #fafafa; outline: none; transition: border-color .2s, box-shadow .2s, background .2s; }
    ...
    .rekening-box { background: #f9f8f7; border: 1px solid #f0eeeb; border-radius: 14px; padding: 16px 18px; }

    .upload-zone { border: 2px dashed #e0e0e0; border-radius: 14px; padding: 0; overflow: hidden; position: relative; background: #fafafa; cursor: pointer; transition: border-color .2s, background .2s; }
    ...
    .doc-img { width: 100%; height: 130px; object-fit: cover; display: block; border-radius: 12px 12px 0 0; transition: opacity .2s; }
    .upload-zone:hover .doc-img { opacity: .75; }
    .upload-zone-body { padding: 14px 16px; display: flex; align-items: center; gap: 12px; }
    .upload-zone-icon { width: 38px; height: 38px; border-radius: 10px; background: #eff6ff; border: 1.5px solid #bfdbfe; display: flex; align-items: center; justify-content: center; font-size: 18px; flex-shrink: 0; }
    ...
    .doc-preview-live { width: 100%; height: 130px; object-fit: cover; display: none; border-radius: 12px 12px 0 0; }

    .btn-submit { width: 100%; background: #2563eb; color: #fff; font-size: 14px; font-weight: 800; font-family: 'Plus Jakarta Sans', sans-serif; border: none; border-radius: 13px; padding: 14px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; box-shadow: 0 4px 16px rgba(37,99,235,.3); transition: background .2s, transform .1s, box-shadow .2s; margin-top: 20px; }
...
<div class="dok-wrap">
    ...
    <div class="status-banner banner-warning">
        ...
        <div>
            ...
            @if($toko->catatan_dinas)<div class="body">{{ $toko->catatan_dinas }}</div>@endif
    ...
    <div class="card" style="margin-bottom:20px;">
        <div class="section-title" style="justify-content:space-between;margin-bottom:16px;">
            <div style="display:flex;align-items:center;gap:8px;">
                <span class="dot dot-teal"></span> Sertifikat Dinas
            </div>
            <div style="display:flex;gap:8px;">
                <a href="{{ route('penjual.toko.sertifikat') }}" target="_blank"
                    style="background:#f0fdfa;color:#059669;border:1px solid #99f6e4;font-size:11px;font-weight:700;padding:6px 12px;border-radius:8px;text-decoration:none;display:flex;align-items:center;gap:5px;">
                ...
                <a href="{{ route('penjual.toko.sertifikat') }}"
                    style="background:#2563eb;color:#fff;font-size:11px;font-weight:700;padding:6px 12px;border-radius:8px;text-decoration:none;display:flex;align-items:center;gap:5px;">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px;">
            <div style="background:#f9f8f7;border:1px solid #f0eeeb;border-radius:12px;padding:12px 14px;">
                <div style="font-size:10px;font-weight:800;color:#bbb;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px;">No. Sertifikat</div>
                <div style="font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:600;color:#1a1a1a;">{{ $toko->no_sertifikat ?? '-' }}</div>
            </div>
            <div style="background:#f9f8f7;border:1px solid #f0eeeb;border-radius:12px;padding:12px 14px;">
                <div style="font-size:10px;font-weight:800;color:#bbb;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px;">Tanggal Terbit</div>
                <div style="font-size:12px;font-weight:600;color:#1a1a1a;">{{ $toko->tanggal_sertifikat?->format('d M Y') ?? '-' }}</div>
            </div>
            <div style="background:#f9f8f7;border:1px solid #f0eeeb;border-radius:12px;padding:12px 14px;">
                <div style="font-size:10px;font-weight:800;color:#bbb;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px;">Berlaku Hingga</div>
                @php $exp = $toko->kadaluarsa_sertifikat; @endphp
                <div style="font-size:12px;font-weight:600;color:{{ $exp && $exp->isPast() ? '#dc2626' : '#059669' }};">
                    {{ $exp?->format('d M Y') ?? '-' }}
                    @if($exp && $exp->isPast()) <span style="font-size:10px;">⚠ Kadaluarsa</span> @endif
                </div>
            </div>
        </div>
      ...
      {{-- PDF Preview (collapsible) --}}
        <div style="margin-top:4px;">
            <button type="button" onclick="toggleSertifikat()"
                style="width:100%;background:#f9f8f7;border:1.5px solid #e8e8e8;border-radius:12px;padding:10px 16px;font-family:'Plus Jakarta Sans',sans-serif;font-size:12px;font-weight:700;color:#555;cursor:pointer;display:flex;align-items:center;justify-content:space-between;gap:8px;">
                <span>👁 Tampilkan Preview Sertifikat</span>
                <span id="sert-toggle-icon">▼</span>
            </button>
            <div id="sert-preview-box" style="display:none;margin-top:8px;border-radius:14px;overflow:hidden;border:1.5px solid #e8e8e8;">
                <iframe
                    src="{{ route('penjual.toko.sertifikat') }}"
                    style="width:100%;height:400px;border:none;display:block;"
                    title="Preview Sertifikat Dinas">
                </iframe>
            </div>
        </div>
    </div>
                        ...
                        <div class="upload-zone {{ $toko->foto_ktp ? 'has-file' : '' }}" id="zone-ktp">
                            ...
                            <div class="upload-zone-body">
                                ...
                                <div class="upload-zone-text">
                                    ...
                                    @if($toko->foto_ktp)<div class="ok">Foto sudah diunggah · klik untuk ganti</div>
                                    @else<div class="hint">JPG, PNG · Maks. 2MB</div>@endif
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="upload-zone {{ $toko->foto_usaha ? 'has-file' : '' }}" id="zone-usaha">
                            ...
                            <div class="upload-zone-body">
                                ...
                                <div class="upload-zone-text">
                                    ...
                                    @if($toko->foto_usaha)<div class="ok">Foto sudah diunggah · klik untuk ganti</div>
                                    @else<div class="hint">Eksterior/interior tempat usaha · Maks. 2MB</div>@endif
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="upload-zone {{ $toko->foto_produk_sample ? 'has-file' : '' }}" id="zone-sample">
                            ...
                            <div class="upload-zone-body">
                                ...
                                <div class="upload-zone-text">
                                    ...
                                    @if($toko->foto_produk_sample)<div class="ok">Foto sudah diunggah · klik untuk ganti</div>
                                    @else<div class="hint">Foto produk unggulan toko · Maks. 2MB</div>@endif
                                </div>
                            </div>
                        </div>
                    </div>
...
function toggleSertifikat() {
    const box  = document.getElementById('sert-preview-box');
    const icon = document.getElementById('sert-toggle-icon');
    const btn  = box.previousElementSibling;
    if (box.style.display === 'none') {
        box.style.display = 'block';
        icon.textContent  = '▲';
        btn.querySelector('span').textContent = '👁 Sembunyikan Preview';
    } else {
        box.style.display = 'none';
        icon.textContent  = '▼';
        btn.querySelector('span').textContent = '👁 Tampilkan Preview Sertifikat';
    }
}
...
document.querySelectorAll('.upload-zone input[type="file"]').forEach(input => {
    input.addEventListener('change', function () {
        ...
        reader.onload = ev => { liveImg.src = ev.target.result; liveImg.style.display = 'block'; };
